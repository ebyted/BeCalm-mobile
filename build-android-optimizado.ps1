# Script optimizado para build de Android desde ru} else {
    Write-Host ""
...
if (-not (Test-Path "package.json")) {
    Write-Host "ERROR: No se encuentra package.json. Asegúrate de estar en C:\BeCalm"
    exit 1
}
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "El APK debería estar en: android\app\build\outputs\apk\release\"
    
    # Verificar APK
    $apkPath = "app\build\outputs\apk\release"
    if (Test-Path $apkPath) {
        Get-ChildItem $apkPath -Filter "*.apk" | ForEach-Object {
            Write-Host "APK encontrado: $($_.Name) ($('{0:N2}' -f ($_.Length / 1MB)) MB)"
        }
    }
} else {
    Write-Host ""
